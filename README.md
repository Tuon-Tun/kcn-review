...
## Cài trên máy mới (chuyển giao)
1. Cài [Git](https://git-scm.com), [Python 3.10+](https://python.org) (khi cài Python
   nhớ tick "Add to PATH"), và [Claude Code](https://claude.com/claude-code).
2. Mở Claude Code, đăng nhập tài khoản Claude (gói Pro trở lên).
3. Clone repo: `git clone https://github.com/Tuon-Tun/kcn-review.git`
   (repo private — chủ repo phải thêm bạn làm collaborator trước:
   Settings → Collaborators → Add people).
4. Cài Flask: `pip install -r webapp/requirements.txt`
5. Bấm đúp `start-web.bat` → trình duyệt tự mở `http://127.0.0.1:8765`.

Web app tự tìm Claude Code tại `%LOCALAPPDATA%\AnthropicClaude\claude.exe` hoặc
lệnh `claude` trong PATH — nếu cài chỗ khác, sửa `CLAUDE_CANDIDATES` đầu file
`webapp/app.py`.
